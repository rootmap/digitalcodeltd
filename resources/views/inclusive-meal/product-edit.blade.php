...
@section('title','Edit Inclusive Meal Info')
@section('barcum')
<h1>
    Edit Inclusive Meal
...
<ol class="breadcrumb">
    <li><a href="{{url('admin-ecom/inclusive-meal')}}"><i class="fa fa-tags"></i> Inclusive Meal Info</a></li>
    <li><a href="{{url('admin-ecom/inclusive-meal/create')}}" class="active"> Edit  Inclusive Meal</a></li>
</ol>
@endsection
...
<div class="row">
    ...
    <div class="col-md-12">
        ...
        <div class="box box-danger">
            ...
            <form method="post" role="form" enctype="multipart/form-data" action="{{url('admin-ecom/inclusive-meal/update/'.$edit->id)}}">
                <div class="box-body">
                	<div class="row">
                		<div class="col-md-6">
                			<div class="form-group">
		                        <label for="exampleInputEmail1">Product Name</label>
		                        <input value="{{$edit->name}}" type="text" class="form-control" id="proName" name="name" placeholder="Enter Name">
		                    </div>
                		</div>
                		<div class="col-md-6">
                			<div class="form-group single">
		                        <label for="exampleInputEmail1">Default New Price</label>
		                        <input value="{{$edit->price}}"  type="text" class="form-control" id="exampleInputEmail1" name="price" placeholder="Enter New Price">
		                    </div>
                		</div>
                	</div>
                                       ...
                                       <td>
                                           <table width="100%" class="table table-bordered table-responsive">
                                               <thead>
                                                   <tr>
                                                       <th colspan="4">
                                                         <input type="text" name="pro_opt_name[]" placeholder="Require | Options | With" class="form-control" value="{{$row->pro_opt_name}}">
                                                         <input type="hidden" name="pro_opt_fid[]" value="{{$k}}">
                                                       </th>
                                                   </tr>
                                               </thead>
                                               <tbody>
                                                   @if(count($row->option_param)>0)
                                                     <?php $kkk=1; ?>
                                                     @foreach($row->option_param as $rr)
                                                       <tr>
                                                           <td valign="top">{{$kkk}}</td>
                                                           <td valign="top">
                                                               <input type="text"  name="s_name_{{$k}}[]" placeholder="Name" class="form-control" value="{{$rr->name}}">
                                                           </td>
                                                           <td valign="top">
                                                               <input type="text" name="s_price_{{$k}}[]" placeholder="Price" class="form-control" value="{{$rr->price}}">
                                                           </td>
