<script>
        ...
        if(discount_rate)
        {
        	...
        	if(discount_type)
        	{
        		...
        		if(limit_check>0 && totalPrice<limit_check)
        		{


        			$(".discount-space").fadeOut();
        			var amount_to_get_discount=limit_check-totalPrice;
        			discount='0.00';
        		...
        		else 
        		{
        			...
        			discount     	= parseFloat((totalPrice*actual_discount_rate)/100).toFixed(2);
        ...
        if(obj.rec!==null)
        {
            var discount_rate=$(".spend-discount-colndel").attr("data-discount");
            console.log('Current Discount Rate',discount_rate);
                if(discount_rate)
                {
                    ...
                    if(discount_type)
                    {
                        ...
                        var limit_check=$(".spend-discount-colndel").attr("data-disamount-limit");
                        ...
                        if(limit_check>0 && totalPrice<limit_check)
                        {
                            ...
                            discount='0.00';
                            var discountTextString='Spend Another £'+parseFloat(amount_to_get_discount).toFixed(2)+' get '+discount_rate+' discount';
                            $(".spend-discount-colndel").children("div:eq(0)").children("p:eq(0)").html(discountTextString);
                        }
                        else 
                        {
                            $(".discount-space").fadeIn();
            ...
            var totalSubPrice=((totalPrice-0)+(tax-0))-discount;
        ...
        function updateLayoutOne(obj)
        {
             ...
             $(".mini-cart-list").children("#"+rowID).children("div").children("a").children(".cartItemPricePR").html(obj.price);
            ...
            if(parseObj.product_level_type==2)
            {
                ...
                //console.log($(".mini-cart-list").children("#"+rowID).children("div").children("div").children(".cart-extra-mini-box").html());
        ...
        function minilayoutOne(obj)
        {
        	...
        	var dataSTRExtras ='';
                ...
                dataSTR+='   <a class="cart-item-highlight" href="javascript:void(0);"><span class="cartQUANIT" style="color:#777;">'+obj.qty+'</span> X '+parseObj.name;
                ...
                dataSTRExtras+='         </div>';
                var totalPriceGen=(obj.price-0)+(extraPrice-0);
                dataSTRExtras+='        </div>';
        ...
        function delRowLi(rowID)
        {
            ...
            $("#"+liID).remove();
            var addtoCartURL="<?php echo e(url('order-item/delete-to-cart')); ?>";
            //------------------------Ajax POS Start-------------------------//
            $.ajax({
                'async': false,
                'type': "POST",
        ...
        {
            var addtoCartURL="<?php echo e(url('order-item/add-to-cart/json')); ?>";
            //------------------------Ajax POS Start-------------------------//
            $.ajax({
                'async': false,
                'type': "GET",
        ...
        function fixCartScroll()
        {
            ...
            $(document).ready(function(){
                ...
                $("input[name=rec]").click(function(){
                    ...
                    if(selecVal!='')
                    {
                        ...
                        $.ajax({
                            ...
                            'dataType': 'json',
                            'url': addtoCartURL,
                            'data': {'rec':item_id,'_token':"<?php echo e(csrf_token()); ?>"},
                            'success': function (data) {
                                //tmp = data;
                                console.log("Processing : "+data);
                                loadCart(data);
                ...
                $(".checkout-page").click(function(){
                    var addtoCartURL="<?php echo e(url('order-item/add-to-cart/typeofdelivery')); ?>";
                    //------------------------Ajax POS Start-------------------------//
                    $.ajax({
                        'async': true,
                        'type': "GET",
                ...
                $(".empty-cart").click(function(){
                    var addtoCartURL="<?php echo e(url('order-item/cart/clear')); ?>";
                    //------------------------Ajax POS Start-------------------------//
                    $.ajax({
                        'async': true,
                        'type': "GET",
                ...
                $('.executive-set-menu').click(function(){
                    ...
                    $.each(execArray,function(index,row){
                        var rowData=[];
                            rowData['value']=$(row).val();
                            rowData['name']=$(row).attr('title');
